...
public class SparkMax extends CANSparkMax {
  ...
  public SparkMax withInitializer(BiFunction<CANSparkMax, Boolean, Boolean> initialize) {
    ...
    Logger.tag("Spark Max").debug("Attempting configuration for SparkMax with ID {}", getDeviceId());
    ...
    if(successful) {
      ...
      if(setAttemptNumber > 0) {
        Logger.tag("Spark Max").debug("Spark Max ID {}: Configuration took {} tries.", getDeviceId(), setAttemptNumber + 1);
      }
    }
    return this;
  }
